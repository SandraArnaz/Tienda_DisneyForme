<?php

require_once('app/models/shoppingCartItem.php');

class ShoppingCart {
	public $items;

	function __construct(){
		$this->items = [];
	}

	////Añade el producto que le pasamos a la lista vacia que tenemos como atributo en la clase
	function addProduct($product) {
		$newItem = new ShoppingCartItem($product);
		$items = [];
		$hasAdded = false;

		foreach ($this->items as $item) {
			if ($item->equals($newItem)) {
				$item->increment();
				$hasAdded = true;
			}
			$items = array_merge($items, [$item]);
		}

		if (!$hasAdded) {
			$items = array_merge($items, [$newItem]);
		}

		$this->items = $items;
	}

	//Elimina el producto que le pasamos de la lista que tenemos como atributo en la clase
	function deleteProduct($product) {
		$newItem = new ShoppingCartItem($product);
		$items = [];

		foreach ($this->items as $item) {
			if ($item->equals($newItem)) {
				$item->decrement();
			}
			if ($item->cantidad !== 0) {
				$items = array_merge($items, [$item]);
			}
		}

		$this->items = $items;
	}

	//Devuelve el numero de productos que se encuentran dentro de la lista
	function numberProducts() {
		$numberProducts = 0;

		foreach ($this->items as $item) {
			$numberProducts += $item->cantidad;
		}

		return $numberProducts;
	}
	
	//Devuelve el precio total de la suma de todos los productos de la lista
	function totalPrice() {
		$totalPrice = 0;

		foreach ($this->items as $item) {
			$totalPrice += $item->totalPrice();
		}

		return $totalPrice;
	}
}

?>